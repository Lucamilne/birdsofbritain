...
birds.data = {
    ...
    "Raven": {
        ...
        "conservationStatus": "Green",
        "protectedBy": "The Wildlife and Countryside Act 1981",
        "description": "The raven is a big black bird, a member of the crow family. It is massive - the biggest member of the crow family. It is all black with a large bill, and long wings. In flight, it shows a diamond-shaped tail. Ravens breed mainly in the west and north although they are currently expanding their range eastwards. Most birds are residents, though some birds - especially non-breeders and young birds - wander from their breeding areas but do not travel far.",
    ...
    "Sanderling": {
        ...
        "conservationStatus": "Amber",
        "protectedBy": "The Wildlife and Countryside Act 1981",
        "description": "The sanderling is a small, plump, energetic wading bird. It has a short straight black bill and Regular black legs. It is pale grey above and white underneath, and there is a black mark at its shoulder where the folded wing meets the body. It does not breed in the UK, but is a winter visitor and passage migrant in spring and autumn, journeying to and from their high Arctic breeding grounds.",
    ...
    "Yellow wagtail": {
        ...
        "conservationStatus": "Red",
        "protectedBy": "The Wildlife and Countryside Act 1981",
        "description": "The yellow wagtail is a small, graceful, yellow and green bird, with a medium-length tail and slender black legs. It spends much time walking or running on the ground. As its name implies, it wags its tail from time to time. It is a summer visitor, migrating to winter in Africa. It breeds in a variety of habitats in the UK, including arable farmland, wet pastures and upland hay meadows. Serious declines in breeding numbers across all of these habitats place the yellow wagtail on the red list of birds of conservation concern.",
...
birds.searchCharacteristics = function (habitat, featherColor, beak) {
    ...
    birds.listOfBirds().forEach(el => {
        ...
        if (bird) {
            if (featherColor.every(el => bird.featherColor.indexOf(el) > -1) && bird.Beak.includes(beak) && bird.habitat.includes(habitat)) {
                birdArray.push(el);
            }
        }
    })
...
birds.birdsByHabitat = function () {
    let birdsByHabitat = new Object();

    birds.listOfFeatures.habitats.forEach(habitat => birdsByHabitat[habitat] = []);

    for (const bird in birds.data) {
        if (birds.data[bird].features && birds.data[bird].features.habitat) {
            birds.data[bird].features.habitat.forEach(habitat => birdsByHabitat[habitat].push(birds.data[bird].name));
        } else {
            continue;
        }
    }

    return birdsByHabitat;
}
...
birds.birdsByLetter = function() {
    ...
    for (var i = 0; i < 26; i++) {
        ...
        birdsByLetter[letter] = [];
    }

    listOfBirds.forEach(bird => {
        const letter = bird.charAt(0)
        birdsByLetter[letter].push(bird);
    })

    return birdsByLetter;
}
